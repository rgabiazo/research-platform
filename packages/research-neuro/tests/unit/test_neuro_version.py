...
from research_platform.neuro import _version


def test_package_version_queries_research_neuro_distribution(monkeypatch) -> None:
    requested: list[str] = []
    ...
    assert requested == ["research-neuro"]
...
def test_package_version_uses_source_checkout_fallback_when_metadata_is_unavailable(monkeypatch) -> None:
    ...
    assert _version.DISTRIBUTION_NAME == "research-neuro"
    assert _version.package_version() == "0.1.0a1"
