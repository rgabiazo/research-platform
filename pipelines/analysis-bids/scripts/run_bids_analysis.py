#!/usr/bin/env python3
"""Thin pipeline runner for adapter-driven BIDS analysis execution."""
...
from research_platform.core.runtime_plan import (  # noqa: E402
    execute_runtime_plan_steps,
    execute_runtime_plan_unit,
    load_runtime_plan,
    runtime_plan_unit_ids,
    write_runtime_plan_marker,
)
from research_platform.core.tool_adapters import load_bids_analysis_tool_adapter  # noqa: E402


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Plan or execute adapter-driven BIDS analysis commands.")
    ...
    if prep_exit_code != 0:
        return prep_exit_code
    exit_code = execute_runtime_plan_steps(plan.get("steps", []), cwd=WORKSPACE_ROOT)
    if exit_code == 0:
        write_runtime_plan_marker(marker_path, step_count=len(plan["steps"]), unit_count=len(runtime_plan_unit_ids(plan)))
    return exit_code
...
def _load_or_build_plan(
    *,
    ...
    adapter = load_bids_analysis_tool_adapter({"adapter": tool["adapter"]})
...
def _validate_local_plan_only_manifest(
    *,
    ...
    raise SystemExit(
        ...
        "For local plan validation, use `rp run plan analysis bids` to create a local manifest."
    )
